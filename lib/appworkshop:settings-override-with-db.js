...
var SettingsOverride = new Meteor.Collection(COLLECTION_NAME);
Settings = {};
if (Meteor.isClient) {
  Settings = new Ground.Collection('Meteor_Settings_Grounded');
  Settings.observeSource(SettingsOverride.find());
  Meteor.subscribe('Meteor_Settings_Grounded', () => {
    Settings.keep(SettingsOverride.find()); // Match grounded data with subscription or a subset
  });

  // You might also want to point Meteor_Settings_Override finds to Settings - this is a custom case depending
  // on implementation
  SettingsOverride.find = function() {
    return Settings.find(arguments);
  };
  SettingsOverride.findOne = function() {
    return Settings.findOne(arguments);
  };
...
if (Meteor.isServer) {
  Meteor.startup(function() {
    ...
    // If not, seed the DB with an empty object
    var settingsObj = Settings.findOne();
    if (!settingsObj) {
      // seed our DB with an empty object
      Settings.insert({},
